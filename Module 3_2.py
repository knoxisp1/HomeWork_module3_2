...
def send_email(message, recipient, sender="university.help@gmail.com"):  # Создаем функцию send_email
    recipient = str(recipient)  # Переводим recipient в строку
    a = sender  # Обьявлем переменную а,значению sender
    if "@" not in recipient or "@" not in sender or not sender.endswith(
            (".com", ".ru", ".net")) or not recipient.endswith((".com", ".ru", ".net")):  # Проверяем условия
        print(f"Невозможно отправить письмо с адреса {sender} на адресc {recipient}")
    # Выводим сообщение в строку если не содержаться символы @ или не заканчиваться на на ".com"/".ru"/".net"
    elif sender == recipient:  # Если же sender и recipient совпадают
        print(f"Нельзя отправить письмо самому себе")  # Выводим следующую строку
    elif a == "university.help@gmail.com":  # Если же отправитель по умолчанию:
        print(f"Письмо успешно отправлено с адреса {sender} на адрес {recipient}.")  # Выводим следующую строку
    else:  # Иначе
        print(
            f"НЕСТАНДАРТНЫЙ ОТПРАВИТЕЛЬ! Письмо отправлено с адреса {sender}на адрес {recipient}.")  # Выводим следующую строку.
